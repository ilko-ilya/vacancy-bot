 ...
 * * Этот сервис был разработан для обхода сложной защиты Cloudflare на DOU.ua
 * через эмуляцию реального браузера (Playwright/Chromium).
 * * Статус: В текущей версии заменен на ScraperAPI для оптимизации
 * оперативной памяти сервера (GCP e2-micro).
 * * Автор пытался выйти из положения всеми доступными методами — и это сработало,
 * но ради экономии ресурсов облака было выбрано более легкое прокси-решение.
 */
